...
class BaseBatchedEnv(gym.Env, abc.ABC):
    ...
    def set_state(self, state):
        lz.log.warning('`set_state` is not implemented')
...
class BaseModelBasedEnv(gym.Env, abc.ABC):
    ...
    def verify(self, n=2000, eps=1e-4):
        dataset = lz.Dataset(lz.rl.gen_dtype(self, 'state action next_state reward done'), n)
        state = self.reset()
        for i in range(n):
        ...
        lz.log.info('rewarder difference: %.6f', l_inf)
